...
import { getEffectiveGhostSettings, isLocallyDeletedMessage, unmarkLocallyDeletedMessage } from '../../../util/ghostMode';
...
addActionHandler('apiUpdate', (global, actions, update): ActionReturnType => {
  switch (update['@type']) {
    ...
    case 'updateMessage': {
      ...
      const ghostSettings = getEffectiveGhostSettings(global.currentUserId);
      const shouldSaveEdits = Boolean(ghostSettings.saveEditsHistory);

      if (shouldSaveEdits) {
        const isBot = Boolean(selectBot(global, chatId));
        const allowSaveEdits = !isBot || ghostSettings.saveInBotDialogs;

        if (allowSaveEdits) {
          const prevHistory = currentMessage.editHistory || [];
          const prevText = currentMessage.content?.text;
          const nextText = message.content?.text;

          if (prevText?.text && nextText?.text && prevText.text !== nextText.text) {
            (message as any).editHistory = [
              ...prevHistory,
              {
                date: currentMessage.editDate || currentMessage.date,
                text: prevText,
              },
            ];
            (message as any).isEditedSpy = true;
          } else if (prevHistory.length) {
            (message as any).editHistory = prevHistory;
            (message as any).isEditedSpy = true;
          }
        }
      }
...
export function deleteMessages<T extends GlobalState>(
  ...
  const ghostSettings = getEffectiveGhostSettings(global.currentUserId);
  const shouldSaveDeleted = Boolean(ghostSettings.saveDeletedMessages);

  const externalIdsToSave: number[] = [];
  const localIdsToDelete: number[] = [];

  ids.forEach((id) => {
    if (isLocallyDeletedMessage(id)) {
      localIdsToDelete.push(id);
    } else {
      externalIdsToSave.push(id);
    }
  });

  if (shouldSaveDeleted && externalIdsToSave.length) {
    let hasSaved = false;
    const affectedChatIds = new Set<string>();

    externalIdsToSave.forEach((id) => {
      let targetChatId = chatId;
      if (!targetChatId || !global.messages.byChatId[targetChatId]?.byId[id]) {
        targetChatId = Object.keys(global.messages.byChatId).find((cId) => Boolean(global.messages.byChatId[cId]?.byId[id]));
      }
      if (!targetChatId) {
        targetChatId = selectCommonBoxChatId(global, id);
      }

      if (targetChatId) {
        const isBotChat = Boolean(selectBot(global, targetChatId));
        const allowSave = !isBotChat || ghostSettings.saveInBotDialogs;

        if (allowSave) {
          const message = selectChatMessage(global, targetChatId, id);
          if (message) {
            hasSaved = true;
            affectedChatIds.add(targetChatId);
            global = updateChatMessage(global, targetChatId, id, {
              isDeleting: false,
              isDeletedSpy: true,
            });
          }
    ...
    if (hasSaved) {
      setGlobal(global);
      affectedChatIds.forEach((cId) => {
        actions.requestChatUpdate({ chatId: cId });
      });
  ...
  if (chatId) {
    ...
    setTimeout(() => {
      ...
      const stillDeletedIds = ids.filter((id) => {
        const msg = selectChatMessage(global, chatId, id);
        return msg?.isDeleting && !msg?.isDeletedSpy;
      });
      if (stillDeletedIds.length) {
        global = deleteChatMessages(global, chatId, stillDeletedIds, { forceDelete: true });
        setGlobal(global);
        actions.requestChatUpdate({ chatId });
      }
  ...
  ids.forEach((id) => {
    ...
    if (commonBoxChatId) {
      ...
      setTimeout(() => {
        ...
        if (msg?.isDeleting && !msg?.isDeletedSpy) {
          global = deleteChatMessages(global, commonBoxChatId, [id], { forceDelete: true });
          setGlobal(global);
