...
import {
  getEffectiveGhostSettings,
  isAdsDisabled,
  isGhostActionBlocked,
  markLocallyDeletedMessages,
} from '../../../util/ghostMode';
import {
...
import { isChatAdmin, isChatGroup } from '../../helpers/chats';
...
addActionHandler('loadMessage', async (global, actions, payload): Promise<void> => {
  ...
  if (!chat) {
    actions.fetchChat({ chatId });
    global = getGlobal();
    chat = selectChat(global, chatId) || ({ id: chatId } as ApiChat);
  }
...
addActionHandler('sendMessage', async (global, actions, payload): Promise<void> => {
  ...
  let effectiveScheduledAt = payload.scheduledAt;
  let effectiveIsSilent = payload.isSilent;

  if (ghostSettings.scheduleMessages && !effectiveScheduledAt && !isStoryReply && type !== 'scheduled') {
    const hasMedia = Boolean(payload.attachments?.length || payload.attachment || payload.sticker || payload.gif);
    effectiveScheduledAt = getServerTime() + (hasMedia ? 25 : 12);
  }

  if (effectiveIsSilent === undefined) {
    if (ghostSettings.sendWithoutSound === 'always') {
      effectiveIsSilent = true;
    } else if (ghostSettings.sendWithoutSound === 'groups_only') {
      if (chat && (isChatGroup(chat) || isChatChannel(chat))) {
        effectiveIsSilent = true;
      }
    }
  }
  ...
  if (!effectiveScheduledAt && !isStoryReply) {
...
addActionHandler('deleteMessages', (global, actions, payload): ActionReturnType => {
  ...
  markLocallyDeletedMessages(messageIds);
...
addActionHandler('sendMessageAction', async (global, actions, payload): Promise<void> => {
  if (isGhostActionBlocked('sendTyping', global.currentUserId)) return;
...
addActionHandler('markMessageListRead', (global, actions, payload): ActionReturnType => {
  if (isGhostActionBlocked('readMessages', global.currentUserId)) return undefined;
...
addActionHandler('forceMarkMessageListRead', (global, actions, payload): ActionReturnType => {
  const { chatId, threadId = MAIN_THREAD_ID, maxId, tabId = getCurrentTabId() } = payload;
  if (!chatId || isLocalMessageId(maxId)) return undefined;
  ...
  // Принудительно отправляем на сервер Telegram в обход режима призрака
  void callApi('markMessageListRead', {
    chat, threadId, maxId, force: true,
  } as any);

  void callApi('markMessagesRead', {
    chat, messageIds: [maxId], force: true,
  } as any);
...
addActionHandler('markMessagesRead', (global, actions, payload): ActionReturnType => {
  if (isGhostActionBlocked('readMessages', global.currentUserId)) return;
  const { chatId, messageIds } = payload;
  const chat = selectChat(global, chatId);
  if (!chat) {
    return;
...
addActionHandler('loadSponsoredMessages', async (global, actions, payload): Promise<void> => {
  if (selectIsCurrentUserFrozen(global) || isAdsDisabled(global.currentUserId)) return;
