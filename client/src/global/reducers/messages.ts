...
import {
  ...
  hasMessageTtl, isMediaLoadableInViewer, isMessageLocal, mergeIdRanges, orderHistoryIds, orderPinnedIds,
} from '../helpers';
...
export function deleteChatMessages<T extends GlobalState>(
  ...
  if (shouldSaveDeleted && !options?.forceDelete) {
    const isBotChat = Boolean(selectBot(global, chatId));
    const allowSave = !isBotChat || ghostSettings.saveInBotDialogs;

    if (allowSave) {
      const messagesToSave: number[] = [];
      ...
      messageIds.forEach((id) => {
        ...
        } else if (message && !isMessageLocal(message)) {
          messagesToSave.push(id);
        } else {
          messagesToDelete.push(id);
        }
      });
      ...
      if (messagesToSave.length) {
        ...
        global = replaceChatMessages(global, chatId, { ...byId });
      }
