    ...
    <div className="settings-content custom-scroll">
      <div className="settings-content-header no-border">
        <AnimatedIconWithPreview
          ...
          noLoop={false}
        />
        <p className="settings-item-description pt-3" dir="auto">{oldLang('lng_settings_experimental_about')}</p>
      </div>
      <Island>
        <ListItem
          href={newAccountUrl}
          icon="add-user"
        >
          <div className="title">{getEthernetString(lang, 'ExperimentalLoginTestServer')}</div>
        </ListItem>
      </Island>
      <Island>
        <ListItem
          ...
          icon="animations"
        >
          <div className="title">{getEthernetString(lang, 'ExperimentalLaunchConfetti')}</div>
        </ListItem>
        <ListItem
        ...
        >
          <div className="title">{getEthernetString(lang, 'ExperimentalStartWave')}</div>
        </ListItem>
        <ListItem
          ref={snapButtonRef}
          onClick={handleSnap}
        ...
        >
          <div className="title">{getEthernetString(lang, 'ExperimentalVaporizeButton')}</div>
        </ListItem>
      </Island>
      <Island>
        <Checkbox
          label={getEthernetString(lang, 'ExperimentalAllowHttpTransport')}
          checked={Boolean(shouldAllowHttpTransport)}
          onCheck={() => setSharedSettingOption({ shouldAllowHttpTransport: !shouldAllowHttpTransport })}
        />
        <Checkbox
          label={getEthernetString(lang, 'ExperimentalForceHttpTransport')}
      ...
      <Island>
        <Checkbox
          label={oldLang('DebugMenuEnableLogs')}
          checked={Boolean(shouldCollectDebugLogs)}
          onCheck={() => setSharedSettingOption({ shouldCollectDebugLogs: !shouldCollectDebugLogs })}
        />
        <Checkbox
          label={getEthernetString(lang, 'ExperimentalEnableSendersDebug')}
        ...
        <ListItem
          ...
          icon="bug"
        >
          <div className="title">{getEthernetString(lang, 'ExperimentalDownloadLog')}</div>
        </ListItem>
      </Island>
    </div>
  );
