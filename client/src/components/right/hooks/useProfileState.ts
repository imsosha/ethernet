...
export default function useProfileState({
  ...
  useEffectWithPrevDeps(([prevTabType]) => {
    if ((prevTabType && prevTabType !== tabType && allowAutoScrollToTabs) || (prevTabType && tabType && forceScrollProfileTab)) {
  ...
  const determineProfileState = useLastCallback(() => {
    ...
    const naturalTop = getTabsNaturalTop(container);
    let state: ProfileState = ProfileState.Profile;
    if (naturalTop > 0 && Math.ceil(container.scrollTop) >= naturalTop - 5) {
