...
const ContextMenuContainer = ({
  ...
  const [isEditHistoryModalOpen, openEditHistoryModal, closeEditHistoryModal] = useFlag();
  ...
  const handleShowEditHistory = useLastCallback(() => {
    closeMenu();
  ...
  const handleMarkAsRead = useLastCallback(() => {
    getActions().forceMarkMessageListRead({
      chatId: message.chatId,
...
export default memo(withGlobal<OwnProps>(
  (global, { message, messageListType, detectedLanguage, threadId }): Complete<StateProps> => {
    ...
    const canMarkRead = !isLocal && !isScheduled && !isInSavedMessages;
    ...
    return {
      ...
      canReply: !message.isDeletedSpy && (message.isEphemeral ? !message.isOutgoing : !isPinned && !isScheduled && canReplyGlobally),
      ...
      canForward: !message.isDeletedSpy && (!message.isEphemeral && !isScheduled && canForward),
      ...
      canMarkRead: !message.isEphemeral && canMarkRead,
