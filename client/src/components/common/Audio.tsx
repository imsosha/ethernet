...
function getCssColor(varName: string, fallback: string): string {
  ...
  if (val.startsWith('var(')) {
    ...
    if (match) return getCssColor(match[1], fallback);
  }
  return val;
}
  ...
  useLayoutEffect(() => {
    ...
    const primaryColor = getCssColor('--color-primary', theme === 'dark' ? '#8774E1' : '#3390EC');
    const linksColor = getCssColor('--color-links', primaryColor);

    const fillColor = getCssColor('--color-voice-waveform', linksColor || primaryColor);
    const fillOwnColor = getCssColor('--color-voice-waveform-own', fillColor);
    const progressFillColor = getCssColor('--color-voice-waveform-played', primaryColor);
    const progressFillOwnColor = getCssColor('--color-voice-waveform-played-own', isOwn && theme === 'dark' ? '#ffffff' : primaryColor);
