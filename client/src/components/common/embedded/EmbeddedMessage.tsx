...
const EmbeddedMessage = ({
  ...
  const senderTitle = sender ? getPeerTitle(oldLang, sender)
    : (replyForwardInfo?.hiddenUserName || (replyForwardInfo as any)?.fromName || senderChatTitle || message?.forwardInfo?.hiddenUserName);
  ...
  function renderTextContent() {
    ...
    if (!message) {
      if (containedMedia?.content?.text) {
        return renderTextWithEntities({
          text: containedMedia.content.text.text,
          entities: containedMedia.content.text.entities,
          asPreview: true,
          emojiSize: EMOJI_SIZE,
        });
      }
  ...
  function renderMediaContentType(media?: MediaContainer) {
    if (!media) return NBSP;
    if (media.content.text) {
      return renderTextWithEntities({
        text: media.content.text.text,
        entities: media.content.text.entities,
        asPreview: true,
        emojiSize: EMOJI_SIZE,
      });
    }
  ...
  function renderSender() {
    ...
    const finalSenderTitle = senderTitle || senderChatTitle;

    if (!finalSenderTitle && !forwardSendersTitle) {
    ...
    return (
      <span className="embedded-sender-wrapper">
        ...
        <span className="embedded-sender">
          {!composerForwardSenders && finalSenderTitle
            && renderText(isReplyToQuote ? oldLang('ReplyToQuote', finalSenderTitle) : finalSenderTitle)}
  ...
  return (
    ...
    >
      ...
      <div className="message-text">
        ...
        <div className={buildClassName('embedded-text-wrapper', isQuote && 'multiline')}>
          {renderTextContent()}
        </div>
      </div>
    </PeerColorWrapper>
