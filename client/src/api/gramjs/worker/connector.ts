...
import { isGhostActionBlocked } from '../../../util/ghostMode';
...
export function callApiLocal<T extends keyof Methods>(
  ...
  if (typeof window !== 'undefined' && window.hermes?.api?._runBeforeRequest) {
    const hookResult = window.hermes.api._runBeforeRequest(fnName, args);
    if (hookResult?.cancel) {
      return Promise.resolve(hookResult.result) as EnsurePromise<MethodResponse<T>>;
    }
    if (hookResult?.args) {
      args = hookResult.args as MethodArgs<T>;
    }
  }
...
export function callApi<T extends keyof Methods>(fnName: T, ...args: MethodArgs<T>): EnsurePromise<MethodResponse<T>> {
  // Ghost Mode Interception
  if (fnName === 'markMessageListRead' || fnName === 'markMessagesRead') {
    const isForce = Boolean((args[0] as any)?.force);
    if (!isForce && isGhostActionBlocked('readMessages')) {
      return Promise.resolve(undefined) as EnsurePromise<MethodResponse<T>>;
    }
  } else if (fnName === 'sendMessageAction') {
    if (isGhostActionBlocked('sendTyping')) {
      return Promise.resolve(undefined) as EnsurePromise<MethodResponse<T>>;
    }
  } else if (fnName === 'updateIsOnline') {
    const isOnline = Boolean(args[0]);
    if (isOnline && isGhostActionBlocked('sendOnline')) {
      return Promise.resolve(undefined) as EnsurePromise<MethodResponse<T>>;
    }
  } else if (fnName === 'viewStory' || fnName === 'markStoryRead') {
    if (isGhostActionBlocked('readStories')) {
      return Promise.resolve(undefined) as EnsurePromise<MethodResponse<T>>;
    }
  }
  ...
  if (fnName === 'sendMessage' || fnName === 'sendMedia' || fnName === 'forwardMessages') {
    if (isGhostActionBlocked('autoOffline')) {
      promise.finally(() => {
        setTimeout(() => {
          void callApi('updateIsOnline', false);
        }, 300);
      });
    }
  }
    ...
    data?.payloads.forEach((payload) => {
      if (payload.type === 'updates') {
        ...
        payload.updates.forEach((update) => {
          ...
          if (typeof window !== 'undefined' && window.hermes?.api?._runOnUpdate) {
            const hookResult = window.hermes.api._runOnUpdate(currentUpdate);
            if (hookResult?.cancel) {
              return;
            }
            if (hookResult?.update) {
              currentUpdate = hookResult.update;
            }
          }
          onUpdate(currentUpdate);
          if (typeof window !== 'undefined' && window.hermes?.api?._runAfterUpdate) {
            window.hermes.api._runAfterUpdate(currentUpdate);
          }
        });
