...
export default function useShowTransition<RefType extends HTMLElement = HTMLDivElement>(
  ...
  useSyncEffectWithPrevDeps(([prevIsOpen]) => {
    ...
    if (isOpen) {
      ...
      const hermesMod = typeof window !== 'undefined' ? (window as any).__hermesMod : undefined;
      const duration = hermesMod?.animationsDisabled
        ? 1
        : (options.closeDuration === CLOSE_DURATION && hermesMod?.animationDuration !== undefined
          ? Number(hermesMod.animationDuration)
