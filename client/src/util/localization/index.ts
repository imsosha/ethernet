...
import ethernetStrings from './ethernetStrings';
...
function getString(langKey: LangKey, count: number) {
  ...
  // Hermes mod strings: русские переопределения для секций мода
  const currentLang = language?.langCode || language?.baseLangCode || language?.pluralCode;
  const isRussian = currentLang ? currentLang.startsWith('ru') : (
    (typeof document !== 'undefined' && document.documentElement.lang?.startsWith('ru'))
    || (typeof navigator !== 'undefined' && navigator.language?.startsWith('ru'))
  );
  if (!langPackStringValue && isRussian && ethernetStrings[langKey]) {
    return ethernetStrings[langKey];
  }
