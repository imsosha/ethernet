...
import {
  addGlobalCallback,
  getActions, getGlobal, removeGlobalCallback, setGlobal,
} from './global';
...
import './assets/fonts/sf-pro.css';
...
async function init() {
  ...
  if (typeof window !== 'undefined') {
    const storeObj = {
      ...
      setGlobal,
      getActions,
      subscribe: (listener: (global: any) => void) => {
        addGlobalCallback(listener);
        return () => {
          removeGlobalCallback(listener);
        };
      },
    };
    if (!window.hermes) {
      (window as any).hermes = {};
    }
    if (!window.ethernet) {
      (window as any).ethernet = {};
    }
    window.hermes!.store = storeObj;
    window.ethernet!.store = storeObj;

    window.dispatchEvent(new CustomEvent('hermes:ready', { detail: { hermes: window.hermes } }));
    window.dispatchEvent(new CustomEvent('ethernet:ready', { detail: { ethernet: window.ethernet } }));
  }
